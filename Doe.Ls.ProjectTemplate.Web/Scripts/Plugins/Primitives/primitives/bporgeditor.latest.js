...
/* /common/init.js*/
/**
 * @preserve Basic Primitives orgEditor Demo v2.0.5
 *
...
primitives.orgeditor.Controller.prototype._getOrgDiagramConfig = function () {
	...
	if (this.options.editMode) {
		...
		if (this.options.readOnlyMode) {
		    if (isMobile) {
		        result.buttons = [
		            new primitives.orgdiagram.ButtonConfig("info", "ui-icon-info"),
		            new primitives.orgdiagram.ButtonConfig("print", "ui-icon-print")
		        ];
		    } else {
		        result.buttons = [
		            new primitives.orgdiagram.ButtonConfig("info", "ui-icon-info", "Info"),
		            new primitives.orgdiagram.ButtonConfig("print", "ui-icon-print", "Print preview")
		        ];
		    }
		} else {
		    if (isMobile) {
		        result.buttons = [
		            new primitives.orgdiagram.ButtonConfig("info", "ui-icon-info"),
		            new primitives.orgdiagram.ButtonConfig("edit", "ui-icon-document"),
		            new primitives.orgdiagram.ButtonConfig("print", "ui-icon-print")
		        ];
		    } else {
		        result.buttons = [
		            new primitives.orgdiagram.ButtonConfig("info", "ui-icon-info", "Info"),
		            new primitives.orgdiagram.ButtonConfig("edit", "ui-icon-document", "Edit"),
		            new primitives.orgdiagram.ButtonConfig("print", "ui-icon-print", "Print preview")
		        ];
		    }
		}
		/*$('span.ui-icon-info').keydown(function (e) {
            alert(1)
		    if (e.which === 13) {
...
primitives.orgeditor.Controller.prototype._onButtonClick = function (e, data) {
	...
	switch (data.name) {
	    ...
	    case "edit":{
	        var win = window.open(window.appUrl + "Position/Edit" + "?Id=" + data.context.id, "_blank");
	        win.focus();
	        break;
	    }
	    case "print": {
	        var win = window.open(window.appUrl + "Position/LoadChartForPrint" + "?id=" + data.context.id, "_blank");
	        win.focus();
	        //$('#myModal').modal('show');
	        break;
	    }
	    case "ViewDesc": {
	        console.log("url" + data.context.DescUrl)
	        if (data.context.DescUrl == "") {
	            alert("Sorry no role/position description attached to this position");
	            break;
	        }
	        var win = window.open(window.appUrl + data.context.DescUrl, "_blank");
	        win.focus();
	        //$('#myModal').modal('show');
	        break;
	    }
	    default:
	        {
	            ...
	            var win = window.open(window.appUrl + "Position/ManageSummary" + "?Id=" + data.context.id, "_blank");
	            win.focus();
	            //}
