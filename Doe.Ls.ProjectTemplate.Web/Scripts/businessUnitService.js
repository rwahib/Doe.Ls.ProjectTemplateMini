...
define(['jquery', 'cnt', 'helper', 'interactive', 'api', 'fValidator'], function ($, cnt, helper, interactive, api) {
                ...
                { 'sName': 'LastModifiedBy', 'mData': 'LastModifiedBy', 'defaultContent': '' },
                {
                    ...
                    'mRender': function (code, ype, full) {
                        ...
                        return '<div class="btn-group-vertical">' +
                                    '<a href="BusinessUnit/Edit?id=' + code + '" class="btn btn-primary pop-up ' + edit + '"  data-toggle="modal" data-target="#vlePluginModal">Edit</a>' +
                                    '<a href="BusinessUnit/Details?id=' + code + '" class="btn btn-primary pop-up ' + read + '" data-toggle="modal" data-target="#vlePluginModal">Detail</a>' +
                                    '<a href="BusinessUnit/Delete?id=' + code + '" class="btn btn-primary pop-up ' + del + '" data-toggle="modal" data-target="#vlePluginModal">Delete</a>' +
                                '</div>';
                    },
                    'bSortable': false
                }
    ...
    function loadFilterLookups() {
        ...
        $('#filterDivisionCode').on('change', function () {
            ...
            $('.table').dataTable().fnDraw();
            var link = $('#create-business-unit');
            link.attr('href', appUrl + 'BusinessUnit/Create?divisionCode=' + $('#filterDivisionCode').val());
        });
        ...
        $('#filterDirectorateId').on('change', function () {
            ...
            var link = $('#create-business-unit');
            link.attr('href', appUrl + 'BusinessUnit/Create?divisionCode=' + $('#filterDivisionCode').val() + '&directorateId=' + $('#filterDirectorateId').val());
        });
    }
