...
define(['jquery', 'cnt', 'helper', 'interactive', 'api', 'fValidator'], function ($, cnt, helper, interactive, api) {
    ...
    return {
        ...
        dtSettings: {
            'iDisplayLength': 10,
            'aoColumns': [
                {'sName':'Code','mData':'Code' },
                {'sName':'MessageFormat','mData':'MessageFormat', 'defaultContent': ''  },
                { 'sName':'MsgCategoryId','mData':'MsgCategoryId', 'defaultContent': ''},
                {'sName':'MessageHint','mData':'MessageHint', 'defaultContent': ''  },
                {
                    'sName':'CreatedDate',
                    'mData': 'CreatedDate',
                ...
                {'sName':'LastModifiedBy','mData':'LastModifiedBy', 'defaultContent': ''  },
                {
                    ...
                    'mRender': function(code){
                        return '<div class="btn-group-vertical">' + 
                                    '<a href="SysMessage/Edit?code=' + code + '" class="btn btn-primary pop-up"  data-toggle="modal" data-target="#vlePluginModal">Edit</a>' + 
                                    '<a href="SysMessage/Details?code=' + code + '" class="btn btn-primary pop-up"  data-toggle="modal" data-target="#vlePluginModal">Detail</a>' + 
                                    '<a href="SysMessage/Delete?code=' + code + '" class="btn btn-primary pop-up"  data-toggle="modal" data-target="#vlePluginModal">Delete</a>' + 
                                '</div>';
                    },
                    'bSortable': false
                }
