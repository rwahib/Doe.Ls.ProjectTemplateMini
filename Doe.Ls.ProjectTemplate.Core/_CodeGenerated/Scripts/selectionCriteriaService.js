...
define(['jquery', 'cnt', 'helper', 'interactive', 'api', 'fValidator'], function ($, cnt, helper, interactive, api) {
    ...
    return {
        ...
        dtSettings: {
            'iDisplayLength': 10,
            'aoColumns': [
                {'sName':'SelectionCriteriaId','mData':'SelectionCriteriaId' },
                {'sName':'Criteria','mData':'Criteria', 'defaultContent': ''  },
                {
                ...
                {
                    'mData': 'SelectionCriteriaId',
                    'mRender': function(code){
                        return '<div class="btn-group-vertical">' + 
                                    '<a href="SelectionCriteria/Edit?id=' + code + '" class="btn btn-primary pop-up"  data-toggle="modal" data-target="#vlePluginModal">Edit</a>' + 
                                    '<a href="SelectionCriteria/Details?id=' + code + '" class="btn btn-primary pop-up"  data-toggle="modal" data-target="#vlePluginModal">Detail</a>' + 
                                    '<a href="SelectionCriteria/Delete?id=' + code + '" class="btn btn-primary pop-up"  data-toggle="modal" data-target="#vlePluginModal">Delete</a>' + 
                                '</div>';
                    },
                    'bSortable': false
                }
