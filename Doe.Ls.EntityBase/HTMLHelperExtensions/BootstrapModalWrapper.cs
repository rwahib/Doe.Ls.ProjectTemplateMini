...
namespace System.Web.Mvc {
    public static class BootstrapModalWrapper {

        public static MvcHtmlString ModalContent(this HtmlHelper html, string modalTitle, string modalBody, string id = null, ButtonHtml[] buttons = null) {
            const string contentFormat = @"<div class='modal fade' id='{0}' tabindex='-1' role='dialog' aria-labelledby='myModalLabel'>
...
";
            id = id ?? "vlePluginModal";

            var modalButtons = new StringBuilder();
            if (buttons == null || buttons.Length == 0)
            {
                buttons = new[]
                {
                    new ButtonHtml
                    {
                        ButtonType = ButtonType.DismessModal,
                        Name = "btn-ok",
                        Value = "Ok"
                    }
                };
            ...
            foreach (var button in buttons)
            {
                switch(button.ButtonType)
                {
                   case ButtonType.Default:
                        modalButtons.Append($"<button type='button' class='btn btn-default' '>{button.Value}</button>");

                        break;

                   case ButtonType.DismessModal:
                        modalButtons.Append(
                            $"<button type='button' class='btn btn-primary' data-dismiss='modal'>{button.Value}</button>");
                        break;

                   case ButtonType.Primary:
                        modalButtons.Append($"<button type='button' class='btn btn-primary' '>{button.Value}</button>");
                        break;

                   case ButtonType.Submit:
                        modalButtons.Append($"<input type='submit' class='btn btn-primary' value='{button.Value}'  />");
                        break;
                }
            ...
            var content = string.Format(contentFormat, id, modalTitle, modalBody, modalButtons);
    ...
    public enum ButtonType {
        Default = 0, Primary = 1, DismessModal = 2, Submit = 3
    }
}
