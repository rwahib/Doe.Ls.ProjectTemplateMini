...
primitives.common.highestContrast = function (baseColor, firstColor, secondColor) {
	...
	if (common.highestContrasts.hasOwnProperty(key)) {
		...
		var hexcolor = baseColor.substring(1);
...
primitives.common.ItemTemplate = function (options, itemTemplateConfig) {
	...
	/*	photoborder = jQuery("<div></div>")
		...
		.css({
			top: "24px",
			left: "10px",
			width: (contentSize.width - 4 - 10) + "px",
			height: "74px"
		...
		if (navigationItem != null) {
			switch (event.which) {
			    case 13: /*Enter*/
				    ...
				    /* Customised locally to meet accessibility in buttons please copy across when update new version*/
				    var newCursorItemId = cursorItemTask.getCursorTreeItem();
				        var target1 = jQuery(event.target);
				        if (target1.hasClass(_data.name + "button") || target1.parent("." + _data.name + "button").length > 0) {
				            button = target1.hasClass(_data.name + "button") ? target1 : target1.parent("." + _data.name + "button");
				            buttonname = button.data("buttonname");
...
primitives.pdf.ItemTemplate = function (options, itemTemplateConfig) {
	...
	/*	doc.rect(position.x + 3, position.y + 24, 50, 60)
		...
		doc.fillColor('black')
			.font('Helvetica', 10)
			.text(itemConfig.description, position.x + 16, position.y + 24, {
				ellipsis: true,
				width: (contentSize.width - 4 - 16),
				height: 74,
				align: 'left'
			});
		doc.restore();
