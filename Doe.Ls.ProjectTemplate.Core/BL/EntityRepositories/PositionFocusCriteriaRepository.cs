...
namespace Doe.Ls.ProjectTemplate.Core.BL.EntityRepositories 
{
    public partial class PositionFocusCriteriaRepository : BaseRepository<PositionFocusCriteria> 
    {
        ...
        public IQueryable<PositionFocusCriteria> ListForPdf()
        {
            return base.List()
                    .Include(ent => ent.LookupFocusGradeCriteria)
                    .Include(ent => ent.PositionDescription)
                    .Include(ent=>ent.LookupFocusGradeCriteria.SelectionCriteria)
                    .OrderBy(ent => ent.PositionDescriptionId);
        }
        ...
        public virtual void AddRange(IEnumerable<PositionFocusCriteria> entities)
        {
            _dbSet.AddRange(entities);
            SaveChanges();
        }
       public void UpdatePositionFocusCriteria(int posDescriptionId, List<PositionFocusCriteria> selFocusList)
        {
            var oldValues =
                this
                    .List()
                    .Where(l => l.PositionDescriptionId == posDescriptionId)
                    .ToList();
           this.RemoveRange(oldValues);
            this.AddRange(selFocusList);
        }
    }
